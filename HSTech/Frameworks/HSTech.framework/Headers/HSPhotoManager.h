...
typedef enum : NSUInteger {
    HSPhotoManagerSelectedTypePhoto = 0, // 只选择图片
    HSPhotoManagerSelectedTypeVideo, // 只选择视频
    HSPhotoManagerSelectedTypePhotoAndVideo // 图片和视频一起
} HSPhotoManagerSelectedType;
...
//------//
@property (assign, nonatomic) HSPhotoManagerSelectedType type;
...
/**
 当选择类型为 HSPhotoManagerSelectedTypePhotoAndVideo 时 此属性为YES时 选择的视频会跟图片分开排  反之  视频和图片混合在一起排
 */
...
/**
 HSPhotoManagerSelectedTypePhoto            // 只选择图片 - 默认类型
 HSPhotoManagerSelectedTypeVideo            // 只选择视频
 HSPhotoManagerSelectedTypePhotoAndVideo    // 图片视频一起选
 
 @param type 选择类型
 @return self
 */
- (instancetype)initWithType:(HSPhotoManagerSelectedType)type;
...
/**
 ...
 @param model 模型
 */
- (void)deleteSpecifiedModel:(HSPhotoModel *)model;
